...
import { Users, BookOpen, Award, TrendingUp, ArrowUpRight, ArrowDownRight, MoreHorizontal, UserPlus, FileText, AlertTriangle, } from "lucide-react";
...
const defaultCourseDistribution = [
...
export default function AdminDashboard() {
    ...
    const enrollmentData = dashboard?.enrollmentData || defaultEnrollmentData;
    const courseDistribution = dashboard?.courseDistribution || defaultCourseDistribution;
    const recentUsers = dashboard?.recentUsers || defaultRecentUsers;
    const alerts = dashboard?.alerts || defaultAlerts;
      ...
      <div className="space-y-6">
          ...
          {stats.map((stat, index) => (<Card key={index} className="bg-card border-border">
              <CardContent className="p-6">
                <div className="flex items-center justify-between">
                  ...
                  <Badge className={stat.changeType === "positive"
                ? "bg-accent/20 text-accent"
                : "bg-destructive/20 text-destructive"}>
                    {stat.changeType === "positive" ? (<ArrowUpRight className="mr-1 h-3 w-3"/>) : (<ArrowDownRight className="mr-1 h-3 w-3"/>)}
                    {stat.change}
                  </Badge>
                </div>
                <div className="mt-4">
        ...
        <div className="grid gap-6 lg:grid-cols-3">
          ...
          <Card className="lg:col-span-2 bg-card border-border">
              ...
              <div className="h-[300px]">
                <ResponsiveContainer width="100%" height="100%">
                  <AreaChart data={enrollmentData}>
                    ...
                    <XAxis dataKey="month" stroke="oklch(0.65 0 0)" tick={{ fill: "oklch(0.65 0 0)", fontSize: 12 }}/>
            ...
            color: "oklch(0.95 0 0)",
                    ...
                    <Area type="monotone" dataKey="students" stroke="oklch(0.65 0.22 260)" strokeWidth={2} fill="url(#colorStudents)"/>
                  </AreaChart>
                </ResponsiveContainer>
              </div>
            </CardContent>
          ...
          <Card className="bg-card border-border">
            ...
            <CardContent>
              <div className="h-[200px]">
                <ResponsiveContainer width="100%" height="100%">
                  <PieChart>
                    <Pie data={courseDistribution} cx="50%" cy="50%" innerRadius={50} outerRadius={80} paddingAngle={2} dataKey="value">
            ...
            color: "oklch(0.95 0 0)",
              ...
              <div className="mt-4 space-y-2">
                {courseDistribution.map((item) => (<div key={item.name} className="flex items-center justify-between text-sm">
        ...
        <div className="grid gap-6 lg:grid-cols-2">
          ...
          <Card className="bg-card border-border">
            ...
            <CardContent>
              <div className="space-y-4">
                {recentUsers.map((user, index) => (<div key={index} className="flex items-center justify-between">
          ...
          <Card className="bg-card border-border">
            ...
            <CardContent>
              <div className="space-y-4">
                {alerts.map((alert, index) => (<div key={index} className="flex items-start gap-3 rounded-lg bg-secondary/50 p-3">
                    <AlertTriangle className={`h-5 w-5 shrink-0 ${alert.type === "warning" ? "text-chart-3" : "text-primary"}`}/>
