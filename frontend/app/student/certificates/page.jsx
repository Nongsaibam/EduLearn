...
export default function CertificatesPage() {
    ...
    const [stats, setStats] = useState({ total: 0, nft: 0, verified: 0 });
    const [error, setError] = useState("");
    useEffect(() => {
        getCertificates({ q: searchQuery })
            .then((data) => {
    ...
    const nftCertificates = useMemo(() => certificates.filter((c) => c.isNFT), [certificates]);
    const regularCertificates = useMemo(() => certificates.filter((c) => !c.isNFT), [certificates]);
    const statCards = [
        { label: "Total Certificates", value: stats.total, icon: <Award className="h-5 w-5"/> },
        { label: "NFT Certificates", value: stats.nft, icon: <Shield className="h-5 w-5"/> },
        { label: "Verified", value: stats.verified, icon: <CheckCircle className="h-5 w-5"/> },
    ];
    ...
    return (<DashboardLayout role="student">
      <div className="space-y-6">
        ...
        <div className="relative">
          ...
          <Input placeholder="Search certificates..." className="pl-9 bg-secondary border-border" value={searchQuery} onChange={(e) => setSearchQuery(e.target.value)}/>
        </div>
        ...
        <Tabs defaultValue="all" className="space-y-6">
          ...
          <TabsContent value="all">
            <div className="grid gap-6 sm:grid-cols-2 lg:grid-cols-3">
              {certificates.map((certificate) => (<CertificateCard key={certificate.id} certificate={certificate} onVerify={() => handleVerify(certificate)}/>))}
            </div>
          </TabsContent>

          <TabsContent value="nft">
            <div className="grid gap-6 sm:grid-cols-2 lg:grid-cols-3">
              {nftCertificates.map((certificate) => (<CertificateCard key={certificate.id} certificate={certificate} onVerify={() => handleVerify(certificate)}/>))}
            </div>
          </TabsContent>

          <TabsContent value="regular">
            <div className="grid gap-6 sm:grid-cols-2 lg:grid-cols-3">
              {regularCertificates.map((certificate) => (<CertificateCard key={certificate.id} certificate={certificate} onVerify={() => handleVerify(certificate)}/>))}
            </div>
          </TabsContent>
        </Tabs>
      </div>
      ...
      <Dialog open={verifyDialogOpen} onOpenChange={setVerifyDialogOpen}>
        <DialogContent>
          ...
          {selectedCertificate && (<div className="space-y-4">
              ...
              <div className="space-y-2 text-sm">
                ...
                {selectedCertificate.isNFT && (<div className="flex justify-between">
