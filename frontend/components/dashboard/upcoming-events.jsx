...
export function UpcomingEvents({ events = defaultEvents }) {
    return (<Card className="bg-card border-border">
      ...
      <CardContent>
        <div className="space-y-3">
          {events.map((event) => (<div key={event.id} className="flex items-start gap-3 rounded-lg p-3 transition-colors hover:bg-secondary/50">
              <div className={`flex h-10 w-10 shrink-0 items-center justify-center rounded-lg ${typeColors[event.type]}`}>
              ...
              {event.course && (<Badge variant="secondary" className="shrink-0 text-xs">
                  {event.course}
                </Badge>)}
            </div>))}
        </div>
