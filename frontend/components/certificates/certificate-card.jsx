...
import { Award, Download, ExternalLink, Share2, CheckCircle, Calendar, } from "lucide-react";
export function CertificateCard({ certificate, onDownload, onShare, onVerify, }) {
    return (<Card className="overflow-hidden bg-card border-border transition-all hover:border-primary/50 hover:shadow-lg">
      ...
      <div className="relative aspect-[1.4/1] bg-gradient-to-br from-primary/20 via-accent/10 to-chart-3/20 p-6">
        ...
        {certificate.isNFT && (<Badge className="absolute right-3 top-3 bg-chart-3/20 text-chart-3">
      ...
      <CardContent className="p-4 space-y-4">
        ...
        <div className="space-y-2">
          ...
          <div className="flex flex-wrap gap-1">
            {certificate.skills.map((skill) => (<Badge key={skill} variant="secondary" className="text-xs">
                {skill}
              </Badge>))}
          </div>
        </div>
