...
export function LeaderboardTable({ entries, currentUserId }) {
    return (<div className="rounded-lg border border-border bg-card overflow-hidden">
      <Table>
        ...
        <TableBody>
          {entries.map((entry) => (<TableRow key={entry.rank} className={`border-border ${entry.rank <= 3 ? "bg-primary/5" : ""} hover:bg-secondary/50`}>
              <TableCell className="font-medium">
                <div className="flex items-center gap-2">
                  {rankIcons[entry.rank] || (<span className="text-muted-foreground w-5 text-center">
                      {entry.rank}
                    </span>)}
                </div>
              </TableCell>
              ...
              <TableCell className="text-right">
                <div className="flex items-center justify-end gap-1">
                  ...
                  {entry.trendValue && (<span className={`text-xs ${entry.trend === "up"
                    ? "text-accent"
                    : entry.trend === "down"
                        ? "text-destructive"
