...
export function AIRecommendationCard({ recommendations = defaultRecommendations }) {
    return (<Card className="bg-card border-border overflow-hidden">
      ...
      <CardContent className="p-0">
        <div className="divide-y divide-border">
          {recommendations.map((rec) => (<div key={rec.id} className="flex flex-col gap-3 p-4 transition-colors hover:bg-secondary/30 sm:flex-row sm:items-center">
              <div className="flex-1 space-y-2">
                <div className="flex flex-wrap items-center gap-2">
                  <h4 className="font-medium text-foreground">{rec.title}</h4>
                  {rec.difficulty && <Badge className={difficultyColors[rec.difficulty] || difficultyColors.Intermediate}>{rec.difficulty}</Badge>}
                </div>
                <p className="text-sm text-muted-foreground">{rec.reason || rec.description}</p>
                <div className="flex flex-wrap items-center gap-4 text-xs text-muted-foreground">
                  {rec.duration && <span className="flex items-center gap-1">
                      <Clock className="h-3 w-3"/>
                      {rec.duration}
                    </span>}
                  {rec.rating && <span className="flex items-center gap-1">
